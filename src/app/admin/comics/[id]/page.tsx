...
export default function EditComicPage() {
  ...
  useEffect(() => {
    async function fetchPage() {
      try {
        const res = await fetch(`/api/admin/comics/${params.id}`);
        if (!res.ok) {
          setError("Comic page not found");
          return;
        }
        const page = (await res.json()) as ComicPage;
        setPageNumber(String(page.pageNumber));
        setTitle(page.title || "");
        setPublishDate(new Date(page.publishDate).toISOString().split("T")[0] || "");
        setStatus(page.status);
        setCommentary(page.commentary || "");
        setContentWarnings(page.contentWarnings);
        setContentWarningOther(page.contentWarningOther || "");
        if (page.imageUrl) {
          setExistingImageUrl(`/api/images/${page.imageUrl}`);
        }
      } catch {
        setError("Failed to load comic page");
      } finally {
        setFetching(false);
      }
    }
  ...
  async function handleDelete() {
    ...
    try {
      const res = await fetch(`/api/admin/comics/${params.id}`, { method: "DELETE" });
      if (!res.ok) {
